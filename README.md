...
Локальная версия предзаполнена контрольным сценарием: 19 августа 2026 года, 09:00, Пулково; рейс на 10:30 перенесён на 18:00; в Москве нужно быть до 18:00.
...
По умолчанию приложение получает реальные предложения через публичный Tutu MCP `https://mcp.tutu.ru/mcp` (Streamable HTTP, без авторизации). Контракт источника изолирован интерфейсом `TravelProvider`: адаптер нормализует ответ MCP в `TravelOffer`, а код приложения самостоятельно проверяет дедлайн и остальные ограничения. При недоступности источника пользователь видит ошибку, а не пустую выдачу.

`FixtureTravelProvider` сохранён для изолированных тестов; демо-данные всегда помечаются как «Демо-данные».

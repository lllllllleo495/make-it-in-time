...
- `checkout_ref` из MCP считается непрозрачным и сохраняется без пересборки; после нажатия «Перейти» отдельный `POST /api/checkout` вызывает `create_checkout_link` и отдаёт готовый deeplink выбранного оффера и тарифа;

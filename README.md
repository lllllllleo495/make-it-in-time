...
- карточки из фактических полей Туту: аэропорты и вокзалы, перевозчик, номер рейса/поезда, длительность, выбранный тариф и багаж;
- строгая фильтрация по текущему времени, дедлайну, цене и типам транспорта;
- до трёх уникальных результатов: с наибольшим запасом, дешевле и с минимальной длительностью в рамках бюджета;
- честный пустой результат, если уложиться нельзя;
- приложенный справочник `support-data.json`: 16 авиакомпаний, 9 продавцов и 32 аэропорта с телефонами, email, примечаниями и официальными ссылками;
...
- три последовательных шага: маршрут, детали и варианты, плюс отдельная помощь с рейсом;
- плавающая кнопка помощи доступна на каждом экране и работает даже до заполнения маршрута;
- промежуточный экран поиска с прогрессом: предложения → проверка прибытия → фильтр по дедлайну;
...
- `checkout_ref` из MCP считается непрозрачным и сохраняется без пересборки; после нажатия «Перейти» отдельный `POST /api/checkout` вызывает `create_checkout_link`; если выданный служебный deeplink требует авторизацию Туту, интерфейс открывает сохранённую поисковую выдачу этого же оффера;

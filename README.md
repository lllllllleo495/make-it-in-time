...
`npm test` собирает production-версию и проверяет основной сценарий, выдачу с одним и тремя вариантами, маленький запас, невозможный дедлайн, ближайший опоздавший маршрут, строгую точку отправления, необязательные данные исходного рейса и валидацию времени.
...
- основная форма из четырёх параметров видна сразу на первом экране;
- исходный рейс и дополнительные ограничения скрыты по умолчанию;
- вертикальная deadline-first выдача с заметным запасом времени;
- поддержка по исходному билету отделена от поиска нового маршрута;
- адаптивный веб-интерфейс для телефона и ноутбука.

Локальный провайдер использует воспроизводимый набор тестовых предложений. Контракт источника изолирован интерфейсом `TravelProvider`; реальный транспортный адаптер должен вернуть тот же нормализованный `TravelOffer`. Это позволяет тестировать продукт без сети и заменить источник без изменения правил отбора или UI.
...
data/                   тестовые предложения и проверенный справочник поддержки

# Техническая документация и архитектура «Успеть»

Этот документ предназначен для разработчиков. Пользовательская инструкция находится в [README](../README.md).

## Локальный запуск

Требуется Node.js `>=22.13.0`.

```bash
npm install
npm run dev
```

Приложение откроется на [http://localhost:3000](http://localhost:3000).

Проверки перед отправкой изменений:

```bash
npm run lint
npm test
```

`npm test` собирает production-версию и запускает интеграционные проверки поиска, дедлайна, поддержки, MCP-нормализации и перехода к билету. Тесты используют `FixtureTravelProvider` и не зависят от внешней сети.

## Главный принцип

LLM может помочь понять свободный текст и объяснить уже рассчитанный результат, но не решает, успевает ли пассажир. Временные и пользовательские ограничения проверяет только детерминированный код.

```mermaid
flowchart LR
  U["Пассажир"] --> UI["Веб-форма"]
  UI --> V["3 поля маршрута + параметры"]
  V --> P["TravelProvider"]
  P --> M["Tutu MCP"]
  P --> F["Fixture для локальной разработки"]
  M --> N["Нормализованные TravelOffer"]
  F --> N
  N --> E["Детерминированный фильтр"]
  E --> S["До 3 уникальных вариантов"]
  UI --> H["POST /api/support"]
  UI --> C["POST /api/checkout"]
  C --> M
  D["Проверенный справочник поддержки"] --> H
  H --> UI
  S --> UI
```

## Поток запроса

1. Первый экран собирает три обязательных параметра: текущую точку, город назначения и дедлайн.
2. Второй экран собирает багаж, пассажиров, бюджет и транспорт. До кнопки «Найти варианты» транспортный источник не вызывается.
3. API повторно валидирует контракт через Zod. Время готовности в контракте равно текущему времени и не запрашивается у пользователя.
4. `TravelProvider` получает структурированный запрос и возвращает предложения без продуктового ранжирования.
5. Каждое предложение нормализуется в `TravelOffer` с абсолютными временем отправления и прибытия, ценой, сегментами, резервной ссылкой и непрозрачным `checkout_ref`.
6. `filterOffers` отбрасывает всё, что уже отправилось, прибывает после дедлайна или нарушает фильтры.
7. `selectRescueOptions` удаляет дубли и выбирает до трёх альтернатив по понятным категориям.
8. Если подходящих вариантов нет, `findNearestAfterDeadline` отдельно находит ближайший маршрут после дедлайна и помечает величину опоздания.
9. При нажатии «Перейти» `POST /api/checkout` передаёт сохранённый `checkout_ref` в `create_checkout_link` и возвращает готовую ссылку Туту для конкретного оффера.
10. На четвёртом экране отдельный `POST /api/support` принимает авиакомпанию, продавца, текущую точку и необязательное время вылета. `getSupportActions()` строит до трёх действий в порядке airline → seller → location. Эти данные не влияют на поиск билетов.

## Где строгая логика

- сравнение текущего времени и отправления;
- сравнение времени прибытия и дедлайна;
- бюджет, виды транспорта и предел пересадок;
- ограничение точки отправления;
- дедупликация;
- выбор до трёх результатов;
- оценка исходного рейса при известном ожидаемом прибытии;
- сопоставление support-actions с сущностями из `support-data.json`;
- приоритет действий airline → seller → location;
- выбор URL по разрешённой цепочке полей справочника.

Эта часть не зависит от LLM и покрыта интеграционными тестами.

## Где допустим LLM

- разобрать фразу пользователя в черновик структурированных полей;
- задать вопрос по конкретному отсутствующему полю;
- кратко объяснить различия уже выбранных вариантов;
- в следующей версии превратить проверенные actions и факты в персонализированное объяснение.

Перед применением извлечённые LLM значения показываются пользователю для подтверждения. LLM не фильтрует билеты, не вычисляет дедлайн, не объединяет отдельные сегменты и не генерирует контакты.

## Источник билетов

`lib/providers/travel-provider.ts` — порт источника данных. В обычном запуске `getTravelProvider()` выбирает `TutuMcpTravelProvider`; `FixtureTravelProvider` включается только переменной `TRAVEL_PROVIDER=fixture` и используется в автоматических тестах.

`TutuMcpTravelProvider`:

- вызывается только после успешной валидации запроса;
- вызывает `search_multitransport` через Streamable HTTP JSON-RPC;
- передаёт `origin`, `destination`, `departure_date`, `adults`, `modes`, `price_max` и уровень `compact`;
- запрашивает до 30 предложений для каждой релевантной даты, максимум для трёх календарных дней;
- принимает составные маршруты только как готовые предложения Туту;
- преобразует `variants[]` в `TravelOffer` без решения «что лучше»;
- не умножает цену на пассажиров повторно: MCP возвращает сумму для всей группы;
- выбирает подтверждённый авиатариф с нужным багажом и подставляет его `offer_hash` в копию `checkout_ref`, поэтому цена карточки и checkout относятся к одному тарифу;
- сохраняет аэропорты/вокзалы, перевозчика, номер рейса или поезда, валюту и ссылки без домыслов;
- сохраняет `search_results_url` как резервный переход, а после выбора карточки вызывает `create_checkout_link`; готовый `checkout_url` транспорта использует напрямую;
- при сетевой ошибке возвращает явную ошибку, а не пустую выдачу;
- передаёт источник `tutu-mcp` для технической диагностики, не перегружая этим пользовательский интерфейс.

Публичный MCP endpoint по умолчанию: `https://mcp.tutu.ru/mcp`. Его можно переопределить переменной `TUTU_MCP_URL`. Для тестового источника используется `TRAVEL_PROVIDER=fixture`.

## HTTP API

### `POST /api/search`

Валидирует `RescueSearchRequest`, запрашивает предложения у `TravelProvider`, повторно проверяет дедлайн и возвращает `SearchResponse` с максимум тремя вариантами.

### `POST /api/checkout`

Получает непрозрачный `checkoutRef` выбранного предложения, вызывает `create_checkout_link` и возвращает ссылку Туту. Если служебный deeplink нельзя надёжно открыть без внутренней сессии Туту, используется сохранённая ссылка поисковой выдачи этого же предложения.

### `POST /api/support`

Получает известные идентификаторы точки, авиакомпании и продавца и строит детерминированный план из `data/support-data.json`. Контакты не генерируются моделью.

## Структура проекта

```text
app/                    интерфейс и HTTP API
data/                   fixture-предложения и справочник поддержки
lib/domain.ts           входные и выходные контракты
lib/search.ts           фильтрация и выбор до трёх вариантов
lib/support.ts          детерминированный план поддержки
lib/providers/          порт, Tutu MCP и fixture-адаптеры
tests/                  интеграционные проверки production-worker
docs/                   документация проекта
worker/                 Cloudflare/Vinext entrypoint
```

## Развёртывание

Приложение содержит серверные обработчики `/api/search`, `/api/checkout` и `/api/support`. Они нужны, потому что браузер не должен выполнять продуктовую логику вместо сервера, а MCP Туту не разрешает cross-origin запросы со статической страницы.

GitHub Pages раздаёт только статические HTML, CSS и JavaScript. Поэтому рабочая схема публикации состоит из двух частей:

```mermaid
flowchart LR
  B["Браузер"] --> P["GitHub Pages: статический интерфейс"]
  P --> A["Публичный HTTPS backend"]
  A --> M["Tutu MCP"]
```

Для production backend должен:

- иметь постоянный публичный HTTPS URL;
- разрешать CORS только домену GitHub Pages;
- иметь исходящий IP, добавленный в allowlist Туту;
- запускать текущие API routes или эквивалентный worker;
- не раскрывать служебные данные MCP в клиентском bundle.

Статический интерфейс собирается командой `npm run build:pages`. Workflow `.github/workflows/deploy-pages.yml` берёт адрес backend из repository variable `API_BASE_URL`. Backend разрешает домен Pages через `ALLOWED_ORIGINS`.

Статическую публикацию нельзя считать готовой, пока `API_BASE_URL` не указывает на развёрнутый backend и не пройден сквозной поиск с GitHub Pages URL.

## Отказоустойчивость

- клиент и сервер валидируют ввод независимо;
- фильтрация не доверяет источнику и повторно проверяет каждое время;
- локальные тестовые данные остаются деталью провайдера и не влияют на правила отбора;
- MCP-вызов ограничен таймаутом 20 секунд;
- `search_multitransport` допускает частичный отказ одного вида транспорта и сохраняет ответы остальных;
- контрактный тест поднимает локальный mock MCP и проверяет аргументы вызова, нормализацию, ссылку и повторную фильтрацию дедлайна;
- пустой ответ и ошибка источника — разные состояния;
- URL и дата проверки хранятся только в `support-data.json`;
- неизвестная сущность создаёт directory miss, но не выдуманную ссылку;
- UI отправляет события `support_block_viewed`, `support_action_clicked` и `support_directory_miss` через локальную analytics-шину;
- production-сборка и критические сценарии запускаются одной командой `npm test`.

## Границы MVP

Не рассчитываются: геолокация, дорога до точки отправления, пробки, регистрация, досмотр, минимальная пересадка, смена аэропорта или вокзала, получение багажа и дорога до конечного адреса. Дедлайн относится к прибытию на вокзал или в аэропорт города назначения.

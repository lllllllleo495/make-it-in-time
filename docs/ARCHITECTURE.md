...
`lib/providers/travel-provider.ts` — порт источника данных. По умолчанию `TutuMcpTravelProvider` обращается к публичному Streamable HTTP endpoint `https://mcp.tutu.ru/mcp` без авторизации. `FixtureTravelProvider` сохраняется для изолированных тестов. Адаптер Tutu MCP:
...
- при сетевой, протокольной или upstream-ошибке возвращать явную ошибку, а не пустую выдачу;
...
- тесты перехватывают HTTP-вызовы MCP и не зависят от внешней сети.

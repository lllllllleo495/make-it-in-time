...
1. Клиент собирает четыре обязательных параметра: текущую точку, город назначения, дедлайн и время готовности. Сведения об исходном рейсе и фильтры необязательны. MCP до отправки формы не вызывается.
...
7. Если подходящих вариантов нет, `findNearestAfterDeadline` отдельно находит ближайший маршрут после дедлайна и помечает величину опоздания.
8. Контакты берутся только из проверенного справочника; генеративная модель не имеет права придумывать телефон или ссылку.
...
- передавать источник `tutu-mcp` для технической диагностики, не перегружая этим пользовательский интерфейс.
